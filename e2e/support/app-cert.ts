 ...
 *
 * Hypercolor web does not verify AppCert signatures (see
 * `certFromHandoffAppKey` / `KeyStore.isAppCertValid`). The simulator still
 * issues a genuine cert so handoff bytes are not random garbage.
 */
...
function encodeCertBody(input: AppCertInput): Uint8Array {
  ...
  // Required keys 0,1,2,4,5,6 — six-entry map. Matches ukd.rs with no optionals.
  return concat(
    Uint8Array.of(0xa6),
    cborUint(0),
