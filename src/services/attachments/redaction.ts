...
export async function reconstructAttachmentWireJson(
  ...
  if (!envelope) {
    throw new AttachmentError(
      "validation",
      "Cannot reconstruct attachment wire JSON",
    );
  }
  ...
  if (!secret) {
    throw new AttachmentError(
      "not-found",
      "Attachment key material is not in KeyStore",
    );
  }
  ...
  if (envelope.thumbnail) {
    if (!secret.thumbnail) {
      throw new AttachmentError(
        "not-found",
        "Thumbnail key material is not in KeyStore",
      );
    }
    live.thumbnail = {
      ...envelope.thumbnail,
...
export function attachmentRawJsonHasLiveSecrets(
  ...
  return secrets.some(
    (value) =>
      value.length > 0 &&
      value !== ATTACHMENT_KEY_PLACEHOLDER &&
      rawJson.includes(value),
  );
}
