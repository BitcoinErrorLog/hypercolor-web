...
const requiredSnippets = [
  ...
  "export function computeInboxKid(",
  "export function sb2Encrypt(",
...
assert.equal(
  sdk.noisePublicKeyFromSecret(aliceNoiseSecret),
  sdk.noisePublicKeyFromSecret(aliceNoiseSecret),
);
...
assert.equal(
  decoder.decode(bob.decrypt(alice.encrypt(encoder.encode(outbound)))),
  outbound,
);
...
assert.equal(
  decoder.decode(alice.decrypt(bob.encrypt(encoder.encode(reply)))),
  reply,
);
...
try {
  ...
  resumeRejected = String(err);
}
assert.ok(
  resumeRejected !== null && /invalid pubky public key/.test(resumeRejected),
  `resumeSessionFromCookie should reject an invalid pubky before I/O; got: ${resumeRejected}`,
);
