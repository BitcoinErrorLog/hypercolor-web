...
- **Amended by ADR 0004:** the first draft of this item also claimed inbound Encrypted Links from people the user follows auto-accept "as `classifyInboundPeer` already specifies." That is no longer true. Upstream pin `a373cd1` narrowed auto-accept to `hasPriorRoutedConversation` and states that `isMutual` / `isFollowing` / `addedManually` "MUST NOT be re-introduced as accept conditions." This tree's copied `wotGate.ts` is the older pin `c7157aaa` and still shows the wider table — re-verified in ADR 0004's audit response, where a review claim to the contrary was rejected on the file. Item 1 therefore delivers badges and ranking only. It is **not** an inbox-enumeration workaround; that is items 3a–3c.
- **Coupled to item 3b (ADR 0004 F-13).** On this tree's current gate, follow / mutual / manual add still auto-accept. Follows import is designed to set `isFollowing` in bulk from a world-readable list, so shipping item 1 on the current gate and then opening the inbox (3b) turns a drop hint naming any followed pubky into an auto-accepted conversation with no request row. **Either the `wotGate.ts` policy sync lands before item 1 ships, or item 1 stays off until 3b's prerequisites are met.** The sync is already a blocking prerequisite of 3b.
...
- **Effort:** M. New secret class (InboxKey custody), new object + validators, send-path descriptor fetch. **Corrected by ADR 0004's audit response:** the vendored `paykit-wasm` does not need rebuilding — `sb2Encrypt` / `sb2Sign` / `computeInboxKid` already exist in the official `0.1.0-rc47` build (HEAD `a2999bf`); only this worktree's rc44 copy is stale, so the task is propagating the pin. Descriptor publish and challenge-decrypt work runs on the current rc44 pin, which already exports `sb2Decrypt`.
- **Custody decided, not open:** Phase A ships native-alias InboxKey custody. A web build that cannot hold the secret outside JS ships only behind a reviewed `SECURITY.md` section, which is a named release gate — see ADR 0004 F-07.
...
- **User-visible value:** A stranger with no follow, no manual add, and no prior link lands in the message-request queue. `collectInboxCandidates` gains a third source: pubkys named by sealed drop hints, and a prune rule so hint-sourced peers do not become permanent probe targets. Accepting opens an ordinary Encrypted Link.
- **Blocking prerequisites (ADR 0004 audit response):** (1) `src/services/link/wotGate.ts` synced to the upstream `a373cd1` policy — otherwise a drop naming a followed pubky auto-accepts (F-13); (2) `routeHeldGroupInbound` must not apply a pending stranger's group `op: 'create'` or recipient-subject `op: 'add'` — otherwise a stranger puts an attacker-titled channel in the channel list before accept (F-15). Neither is optional and neither is a follow-up.
- **Effort:** L. New service (`BitcoinErrorLog/hypercolor-drop`), shared PoW primitive in `pubky-crypto` with wasm + UniFFI bindings, client submit/challenge/fetch/batch-ack, local-state caps and prune rule, request-row rendering constraints.
- **Protocol change:** Yes (`hypercolor.inbox.hint.v1`, the SB2 unlinkable profile, the PoW domain including its hourly freshness bucket). Additive. `chat.message.v0`, the PAM transport, and the group wire kinds are untouched. Device-local: a `links.origin` column and the `WotInput` field rename.
- **Privacy-sensitive:** Yes. New third party, new abuse surface, new crypto profile.
- **Publicly visible:** To the relay operator: an `inbox_kid`, drop counts, timing, constant blob size, submitter and fetcher IPs, and the recipient's own poll traffic. **Not** the sender's pubky (sealed) and **not** any message content (the drop carries a pointer, never a body). The `inbox_kid` → pubky map is public in descriptors, so treat recipient identity as exposed to the relay. To **anyone**, not just the operator: a weak recipient-activity signal via the `503→202` edge on the open submit endpoint, reduced to roughly the ack window by randomized batched acks and randomized relay headroom (ADR 0004 F-03), and accepted as a residual held by the recipient.
- **Bad design if:** The drop carries a message body or preview; a drop auto-accepts; the relay exposes any per-drop fetch/ack state or per-KID warm/cold status (both presence oracles); the queue evicts instead of rejecting within a KID; the relay accepts a client-supplied InboxKey without checking it against the path KID (that is a total inbox takeover — ADR 0004 F-01); local request state is assumed bounded by the relay's cap (it is not — F-02); or pending request rows resolve stranger profiles in the background (F-09).
...
- **Effort:** M after 3b.
- **Protocol change:** Yes (ticket object; descriptor `retired` handling; retired-secret destruction and backup pruning).
- **Privacy-sensitive:** Yes. Tickets name a voucher; rotation changes an address; retired key material must be destroyed rather than archived.
- **Publicly visible:** Descriptor epoch changes and the relay list. The ticket travels sealed.
- **Bad design if:** A "delivered" state for drops appears anywhere; a heartbeat/health signal is added that reveals recipient online status; retired InboxKey secrets are kept past their TTL or accumulate in `/pub/hypercolor.app/v1/backup/latest`, which would make one backup compromise decrypt every epoch and reduce rotation to bookkeeping (ADR 0004 F-07).
...
- **Constrained by ADR 0004 §7**, which is stricter and governs where they overlap: a pending row renders the pubky plus locally-attested data only; resolution is triggered by opening that one row and never by arrival, list render, scroll, or prefetch; resolved fields render as untrusted plain text with a self-asserted label, with no markup, no links, no peer-controlled image loads, and no verified affordance.
- **Bad design if:** Tags auto-accept, every inbound pubky is queried on arrival, or a resolved profile field is rendered with any affordance shared with an accepted contact.
...
| A drop relay that accepts a client-supplied InboxKey without checking it against the path KID | ADR 0004 F-01. The attacker supplies their own key against the victim's KID, decrypts the challenge, and then fetches **and deletes** the victim's entire pending queue. Total inbox takeover from a world-readable descriptor. |
| A drop relay that resolves the recipient's pubky (descriptor, homeserver, or pkarr) in any request path | ADR 0004 F-01. It contradicts the sealed-recipient property and hands the relay a confirmed KID→pubky binding on every fetch. |
| Treating the relay's per-KID cap as a bound on local request state | ADR 0004 F-02. Acking a drop frees the slot, so a flooder cycles indefinitely; local state needs its own cap, TTL, and probe-set prune rule. |
| Background profile or tag resolution for pending request rows | ADR 0004 F-09. Attacker-controlled bytes in the accept prompt, plus an on-arrival read that confirms delivery to the stranger's chosen host. Row-open only. |
| A prompt per-drop ack | ADR 0004 F-03. The `503→202` edge then marks the recipient's sync instant. Acks are batched on a randomized delay; a change that acks promptly for responsiveness is a security regression. |
| Milliseconds in any open-inbox timestamp | ADR 0004 F-10. `Sb2Header.created_at` / `expires_at` are Unix seconds; milliseconds produce a window rejection with no diagnostic pointing at the unit, and a silent PoW difficulty mismatch. |
| Applying a pending stranger's group `op: 'create'` or recipient-subject `op: 'add'` | ADR 0004 F-15. It puts an attacker-titled channel and attacker-authored messages in the channel list before accept. Defer them like chat items. |
...
3a. Toggle on: an unauthenticated `publicGet` from a second browser profile returns the descriptor and `computeInboxKid(inbox_x25519_pub)` equals the published `inbox_kid`. Toggle off: 404. In both states `collectInboxCandidates` output is byte-identical to before and no new peer appears. Sending to a descriptor-less pubky shows **inbox closed**, not a delivered message. Descriptor timestamps are Unix seconds and a validator rejects millisecond-magnitude values. The InboxKey secret is not JS-readable — or the reviewed `SECURITY.md` web-custody section exists and the web toggle is withheld until it does.
3b. A stranger account with no follow, no manual add, and no prior link sends a message; the recipient's next sync shows a **pending request**, not an accepted conversation. Kill the relay before accept — accepting still completes and the first body arrives from the sender's own homeserver path. A drop one bit under `min_pow_bits` returns 400 and never enters the queue. The drop past `WARM_KID_CAP` returns `503 queue-full` with the earlier drops intact. A hint naming an uninvolved third party leaves no request row and no contact row. Plus the audit-response proofs in ADR 0004 Phase B: a challenge request carrying a mismatched InboxKey returns `400 kid-mismatch`; cycling 640 drops holds `message_requests` at the cap and stops probing an unresponsive hint-sourced peer's homeserver after 3 failed syncs; a relay full of junk cold KIDs still serves a new legitimate KID and evicts cold before warm; stale PoW is refused and a boundary-crossing solution is not; the `202` receipt binds `inbox_kid` and `BLAKE3(blob)` and verifies against the descriptor-pinned key; a web-sealed blob round-trips through the Rust relay with identical second-precision timestamps; an over-deep CBOR unknown key is rejected while a flat one still decodes; a pending peer's group PAMs create no channel, no membership row, no notification, and no `link_messages` row, and apply after accept; opening a ten-row pending list issues zero outbound requests while opening one row issues exactly one; two drops to the same recipient differ in `sender_peerid` and neither equals the sender's receiver Noise public key.
3c. With two relays listed, kill relay 1: the drop still lands via relay 2 and is not double-surfaced. Rotate the epoch: drops to the retired KID are discarded locally. At retired-epoch TTL the retired secret is destroyed, its `retired` descriptor entry is gone, and a backup written after rotation contains **exactly one** InboxKey alias. With `require_ticket: true`, an unticketed drop is refused and a ticket from an accepted contact is accepted at `min_pow_bits = 0`. Grep the relay: no route or field exposes per-drop fetch state, delete state, or per-KID warm/cold status. Acks observed at the relay arrive on a randomized delay uncorrelated with the recipient's sync.
